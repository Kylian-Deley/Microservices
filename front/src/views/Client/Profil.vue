  ...
  setup() {
    ...
    const fetchUserData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:3000/api/clients/profil/${clientInfo.id}`, {
          headers: {
    ...
    const updateProfile = async () => {
      try {
        ...
        await axios.put(`http://localhost:3000/api/clients/profil/${clientInfo.id}`, userData.value, {
          headers: {
            'user-id': clientInfo.id
          }
        });
